...
function watchAutoSum_184() {
    ...
    function updateSum() {
        ...
        jQuery(targetID).val(total.toFixed(1)); // Afișăm cu o zecimală
    }

    // Ascultă modificările pe toate cele 3 câmpuri -- Se poate de mai asculta modificarile care pot fi facute si prin alte metode ? De exnplu clear sau alte 
    inputIDs.forEach(selector => {
        jQuery(selector).on('input', updateSum);
    });

    // Execută suma inițial la încărcare
    updateSum();
}
