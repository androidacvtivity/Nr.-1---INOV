...
function validateInitialTotal134() {
    const r1 = document.getElementById("CAPITOL1_R131_C1");
    const r2 = document.getElementById("CAPITOL1_R132_C1");
    const r3 = document.getElementById("CAPITOL1_R133_C1");

    if (!r1 || !r2 || !r3) return; // skip dacă nu găsim câmpurile

    function getSum() {
        const v1 = parseInt(r1.value || r1.textContent || '0', 10);
        const v2 = parseInt(r2.value || r2.textContent || '0', 10);
        const v3 = parseInt(r3.value || r3.textContent || '0', 10);

        return (isNaN(v1) ? 0 : v1) + (isNaN(v2) ? 0 : v2) + (isNaN(v3) ? 0 : v3);
    }

    // Verificare imediată la încărcare
    const total = getSum();
    if (total !== 100) {
        alert(`Cod eroare: 48-004\nSuma valorilor 1.3.1 + 1.3.2 + 1.3.3 este ${total}, dar trebuie să fie exact 100.`);
    }

    // OPTIONAL: adaugă listeneri dacă vrei să se repete în timpul completării:
    [r1, r2, r3].forEach(field => {
        field.addEventListener("input", () => {
            const sum = getSum();
            if (sum !== 100) {
                console.log(`Cod eroare 48-004: suma actuală este ${sum}, nu 100.`);
            }
        });
    });
}
