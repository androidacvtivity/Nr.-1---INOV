...
function validate48_0001() {
    ...
    // ✅ Verifică dacă lipsesc bifările la 1.1.1 / 1.1.2
    ...
    // 🧠 Noua logică: dacă ambele sunt “NU” → trebuie completate 1.5.1 – 1.5.7
    if (r111_nu && r112_nu) {
        const oneFivePairs = [
        ...
        const anyUnselected = oneFivePairs.some(([id1, id2]) =>
            !jQuery(`#${id1}`).is(':checked') && !jQuery(`#${id2}`).is(':checked')
        );
        ...
        if (anyUnselected) {
            webform.errors.push({
                ...
                msg: concatMessage(
                    '48-0001',
                    'Întrebarea 1.5 – Motivul pentru care nu ați lansat produse/servicii',
                    Drupal.t('Cod eroare: 48-0001. Dacă ați selectat “NU” la 1.1.1 și 1.1.2, trebuie completate toate opțiunile DA/NU din 1.5.')
                )
            });
        }
    }
