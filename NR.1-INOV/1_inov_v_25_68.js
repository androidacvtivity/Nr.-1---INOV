(function ($) {
    ...
    Drupal.behaviors.inov1 = {
        attach: function (context, settings) {
            ...
            toggle111_112_M(values);
            check_111_112_M(values);
...
function toggle111_112_M(values) {
    ...
    if (onlyNoFrom111to117 && answeredIn110) {
        toHide.forEach(id => jQuery(id).hide());
        ...
        toHide.forEach(id => jQuery(id).show());
    }
}


function check_111_112_M(values) {
    const watchedIDs = [
        'CAPITOL1_R111_C2', 'CAPITOL1_R112_C2', 'CAPITOL1_R113_C2',
        'CAPITOL1_R114_C2', 'CAPITOL1_R115_C2', 'CAPITOL1_R116_C2',
        'CAPITOL1_R117_C2', 'CAPITOL1_R1101_C1', 'CAPITOL1_R1102_C1', 'CAPITOL1_R1103_C1'
    ];

    watchedIDs.forEach(id => {
        jQuery(`#${id}`).on('change', () => {
            const values = {};
            watchedIDs.forEach(field => {
                values[field] = jQuery(`#${field}`).is(':checked') ? '1' : '0';
            });
            toggle111_112_M(values);
        });
    });
    ...
    const initValues = {};
    watchedIDs.forEach(field => {
        initValues[field] = jQuery(`#${field}`).is(':checked') ? '1' : '0';
    });
    toggle111_112_M(initValues);
}
