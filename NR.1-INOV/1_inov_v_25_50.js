...
function liveInlineValidateCAPITOL1_R134_C1() {
    const field = document.getElementById('CAPITOL1_R134_C1');
    if (!field) return;

    // Creăm containerul pentru mesaj de eroare
    const errorDivId = 'CAPITOL1_R134_C1_error';
    ...
    const observer = new MutationObserver(function () {
        const value = parseInt(field.value || field.textContent, 10);
        const errorBox = document.getElementById(errorDivId);

        if (!isNaN(value) && value !== 100) {
            field.classList.add('has-error');
            errorBox.textContent = "Cod eroare: 48-004 – Valoarea trebuie să fie exact 100%";
