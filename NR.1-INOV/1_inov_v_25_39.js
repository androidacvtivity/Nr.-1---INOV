...
function check_181_logic5() {
    jQuery('#CAPITOL1_R181_C2').change(function () {
        if (jQuery(this).is(':checked')) {
            let inputField = jQuery('#CAPITOL1_R181_C1');
            inputField.val(''); // Clear value
            inputField.prop('checked', false); // Uncheck
            inputField.prop('readonly', true); // Set as readonly
        } else {
            jQuery('#CAPITOL1_R181_C1').prop('readonly', false); // Remove readonly if unchecked
        }
    });
}

function toggle_181_logic5(values) {
    if (values.CAPITOL1_R181_C2 == '1') {
        let inputField = jQuery('#CAPITOL1_R181_C1');
        inputField.val(''); // Clear value
        inputField.prop('checked', false); // Uncheck
        inputField.prop('readonly', true); // Set as readonly
    } else {
        jQuery('#CAPITOL1_R181_C1').prop('readonly', false); // Remove readonly
    }
}
