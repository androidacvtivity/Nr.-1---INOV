...
function check_all(values) {
    jQuery('input[type=checkbox]').change(function () {
        ...
        // Handle mutual exclusivity for Group 1: CAPITOL21_R211_C1, CAPITOL21_R211_C2, CAPITOL21_R211_C3
        var group1 = ["CAPITOL21_R211_C1", "CAPITOL21_R211_C2", "CAPITOL21_R211_C3"];
        if (group1.includes(group)) {
            if (state) {
                jQuery(`input[name="${group1.join('"], input[name="')}"]`).not(this).prop('checked', false);
            }
            return; // Stop further processing
        }

        // Handle mutual exclusivity for Group 2: CAPITOL21_R211_C4, CAPITOL21_R211_C5
        var group2 = ["CAPITOL21_R211_C4", "CAPITOL21_R211_C5"];
        if (group2.includes(group)) {
            if (state) {
                jQuery(`input[name="${group2.join('"], input[name="')}"]`).not(this).prop('checked', false);
            }
            return; // Stop further processing
        }
